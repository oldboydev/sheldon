...
export async function prepareOcrRuntime({ platform, input, output, download, sources = OCR_RUNTIME_SOURCES }) {
  if (!OFFICIAL_PLATFORMS.includes(platform)) {
    throw releaseError('OCR_RUNTIME_PLATFORM_INVALID', `Unsupported OCR runtime platform: ${platform}.`);
  }
  if (typeof download !== 'function') {
    throw releaseError('OCR_RUNTIME_DOWNLOAD_INVALID', 'OCR runtime preparation requires a download callback.');
  }
  assertPinnedOcrRuntimeSources(sources);
  await validateArtifactLayout(input, platform, sources);
...
async function validateArtifactLayout(input, platform, sources) {
  ...
  const executable = platform === 'win32-x64' ? 'tesseract.exe' : 'tesseract';
  await requireRegularFile(join(input, 'runtime', platform, executable), 'OCR_RUNTIME_EXECUTABLE_INVALID');
  await requireEntries(
    join(input, 'runtime', platform),
    ['THIRD_PARTY_NOTICES', executable],
  ...
  for (const model of MODELS) {
    ...
    if (hash !== sources.models[model].sha256) {
      throw releaseError('OCR_RUNTIME_CHECKSUM_INVALID', `OCR model checksum does not match its pinned source: ${model}.`);
    }
  }
}
...
async function requireRegularFile(path, code) {
  try {
    const entry = await lstat(path);
    if (!entry.isFile() || entry.isSymbolicLink() || entry.size === 0) throw new Error('not a non-empty file');
  } catch {
...
async function copyCanonicalFile(source, destination) {
  ...
  try {
    ...
    if (destinationEntry.isSymbolicLink()) {
      throw releaseError('OCR_RUNTIME_OUTPUT_INVALID', `Refusing to write OCR runtime through a symbolic link: ${destination}.`);
    }
  } catch (error) {
