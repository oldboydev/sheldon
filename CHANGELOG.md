...
- Bundles OKF agora distinguem validação strict/lenient para tipos, preservam links deliberadamente quebrados com aviso, fecham links `include` transitivamente e mantêm logs/diffs reproduzíveis entre builds idênticos.
...
- A projeção de relações ignora links Markdown escritos em código ou comentários, a consulta evita materializar relações antes de escolher as raízes e a saída da CLI projeta e serializa no máximo 100 relações por resultado, com sinalização explícita de truncamento. Esse limite não reduz as relações indexadas durante a reconstrução nem a travessia de `query`.
- A projeção limitada de relações aplica o teto antes de materializar a saída, sem penalizar travessias ilimitadas; links Markdown em blocos cercados, código inline ou comentários não criam relações, enquanto blocos indentados permanecem prosa indexável para não apagar conteúdo de listas; destinos com casing diferente no Windows são canonizados, e a recuperação do índice corrompido remove artefatos auxiliares com diagnóstico acionável.
- O build do addon nativo do Windows seleciona explicitamente o Visual Studio 2022 quando nenhum toolchain foi escolhido pelo ambiente, evitando falhas de autodetecção do node-gyp no Node 24.
